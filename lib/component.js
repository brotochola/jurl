window.JURL = {
  ...
  Component: class Component extends HTMLElement {
    ...
    enable() {
      ...
      this.shadowRoot.append(...this.$("template").children)
    ...
    getAllElementsThatAreNotStyleAndNotTheTemplate(){
      return this.root.querySelectorAll(":host > *:not(template):not(style)")
    }
    ...
    disable() {
      ...
      this.$("template").append(...this.getAllElementsThatAreNotStyleAndNotTheTemplate())
    ...
    return 1
      ...
      //by default the root is an element named as the class name of the component
      this.root = this.$(this.constructor.name.toLowerCase());

      if (!this.root) {
        //if the root is not found, then the root is the first element inside the component
        //that is not a style nor a template element
