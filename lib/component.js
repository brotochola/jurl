class Component extends HTMLElement {
  ...
  linkJAttrsWithState() {
    ...
    this.root.querySelectorAll(":host *").forEach((el) => {
      ...
      for (let jAttr of jAttrs) {
        if (jAttr.name.startsWith("j-on")) continue;
  ...
  getState(varName) {
    let ret = (this.state || {})[varName.toLowerCase()];
    // if (ret == undefined) {
  ...
  updateParsedAttributes() {
    ...
    Array.from(this.attributes).map((k) => {
      ...
      if (trimmedValue.endsWith("}}") && trimmedValue.startsWith("{{")) {
        ...
        //I FIRST TRY WITH THIS EVAL STRATEGY:
        //IN CASE YOU PUT SOMETHING LIKE {{something.someVar}}

        let strToEval = "parent.state." + val;
        let i = this.getAttribute("i");
        // if (this instanceof ForTag)
        //   console.log("### ", trimmedValue, this, strToEval);
        try {
          ...
          // console.warn("#####", e);
          //IF IT FAILS I TRY TO GET THE PARENTS' STATE VALUE DIRECTLY
          //IF NOT, THEN JUST PUT UNDEFINED
          this.state[k.name] = parent ? (parent.state || {})[val] : undefined;
        }
        // if (this instanceof ForTag) debugger;
      } else if (decodedAttr) {
    ...
    this.updateJInnerHTML(); //because of uppercase and lower case 'innerHTML' i cannot standarize this with the linkJAttrsWithState method
    this.observer.observe(this, this.observerConfig);
  }
  getAllChildrenComponents() {
    return Array.from(this.root.querySelectorAll("*")).filter(
      (k) => k instanceof Component
    );
  }
  ...
  update() {
    ...
    // if (this instanceof ForTag) this.executeForFunctionality();
  ...
  replaceCurlyBracketsWithContent() {
    ...
    for (let el of elementsWithNoChildrenAndCurlyBracketsContent) {
      if (el.parentNode instanceof ForTag) return;
