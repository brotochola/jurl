  ...
  if (hasCurlyBrackets(value)) {
    ...
    try {
      ...
      if (evalled instanceof Function) {
        //BIND SCOPE TO ARROW FUNCTIONS
        updatedVal = eval(
          "(function(){return " + newValWithNoCurly + "})"
        ).bind(compo)()();
      }
    } catch (e) {}
    ...
    try {
      let evalled = eval(newValWithNoCurly);
      if (evalled instanceof Object || typeof evalled == "string") {
        updatedVal = evalled;
      } else if (evalled == undefined) {
        //IF IT COULD EVAL IT, BUT IT'S UNDEFINED
        //BIND THIS TO GET THE VALUE
        updatedVal = eval(
          "(function(){ return " + newValWithNoCurly + " })"
        ).bind(compo)();
      }
